...
void add_dm_parameter_tolist(struct list_head *head, char *param_name, char *param_data, char *param_type)
{
    ...
    if (param_data)
        dm_parameter->data = strdup(param_data);
    if (param_type)
        dm_parameter->type = param_type ? param_type : "xsd:string";
}
...
static void message_compute_signature(char *msg_out, char *signature)
{
    ...
    struct config *conf;
    conf   = &(cwmp->conf);
    result = HMAC(EVP_sha1( ), conf->acs_passwd, strlen(conf->acs_passwd), msg_out, strlen(msg_out), NULL, NULL);
