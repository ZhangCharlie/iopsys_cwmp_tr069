...
static struct binding_request br_crport           = {.binding_cr = 1, .udp_listen = &listen_crport, .utimer = {.cb = binding_request_crport_cb}};
static struct binding_request br_crport_keepalive = {.is_keealive = 1, .binding_cr = 1, .udp_listen = &listen_crport, .utimer = {.cb = binding_request_crport_cb}};
...
ssize_t timeout_recvfrom(int sock, char *buf, int length, struct sockaddr_in *connection, int timeoutinseconds)
{
    ...
    if (select(sock + 1, &socks, NULL, NULL, &t) && (r = recvfrom(sock, buf, length, 0, ( struct sockaddr * )connection, &clen)) != -1)
    {
        return r;
    }
    else
    ...
    if (r > 0)
    {
        ...
        if (ntohs(sh->type) == BINDING_ERROR)
        {
            ...
            if (udp_listen->br != NULL && stunid_cmp(&(sh->id), &(udp_listen->br->id)) == 0 && code == 401)
            {
                ...
                stun_log(SINFO, "Trying new Binding Request in %ds", (udp_listen->br->auth_fail < 3) ? 0 : (udp_listen->br->auth_fail - 2) * 3);
                uloop_timeout_set(&udp_listen->br->utimer, (udp_listen->br->auth_fail < 3) ? 0 : ((udp_listen->br->auth_fail - 2) * 3000));
                udp_listen->br = NULL;
            }
            else if (code != 401)
            {
