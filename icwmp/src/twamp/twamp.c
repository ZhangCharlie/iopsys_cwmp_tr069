...
static void cleanup_client(struct client_info *client)
{
    ...
    used_sockets--;
    int i;
    for (i = 0; i < client->sess_no; i++) /* If socket is -1 the session has already been closed */
...
static int send_accept_session(struct client_info *client, RequestSession *req)
{
    ...
    if ((used_sockets < 64) && (client->sess_no < MAX_SESSIONS_PER_CLIENT))
    {
        ...
        if (socket_family == AF_INET6)
        {
            ...
            while (check_time-- && bind(testfd, ( struct sockaddr * )&local_addr, sizeof(local_addr)) < 0) local_addr.sin6_port = htons(20000 + rand( ) % 1000);
        ...
        else
        {
            ...
            while (check_time-- && bind(testfd, ( struct sockaddr * )&local_addr, sizeof(struct sockaddr)) < 0) local_addr.sin_port = htons(20000 + rand( ) % 1000);
...
static int receive_test_message(struct client_info *client, int session_index)
{
    ...
    if ((client->sessions[session_index].fw_msg % 10) == 0)
    {
        ...
        // printf("FW Lost packets: %u/%u, FW Loss Ratio: %3.2f%%\n", client->sessions[session_index].fw_lst_msg, client->sessions[session_index].fw_msg,
        // (float)100 * client->sessions[session_index].fw_lst_msg / client->sessions[session_index].fw_msg);
    }
    return rv;
}
...
int twamp_connect(void)
{
    ...
    while (1)
    {
        ...
        uint8_t buffer[4096];
        int     i, j;
        for (i = 0; i < MAX_CLIENTS; i++) /* It can only receive TWAMP-Control messages from Online clients */
            if (clients[i].status != kOffline)
                if (FD_ISSET(clients[i].socket, &tmp_fds))
                {
                    ...
                    {
                        case kConnected:
                            /* If a TCP session has been established and a ServerGreeting has been sent, wait for the SetUpResponse and finish the TWAMP-Control
                             * setup */
                ...
                for (j = 0; j < clients[i].sess_no; j++)
                {
                    ...
                    else
                    {
                        ...
                        twamp_log(SINFO,
                                  ...
                                  ( float )100 * clients[i].sessions[j].fw_lst_msg / clients[i].sessions[j].fw_msg);
                    }
                }
