...
void update_section_list_icwmpd(char *config, char *section, char *option, int number, char *filter, char *option1, char *val1, char *option2, char *val2)
{
    ...
    while (i < number)
    {
        DMUCI_ADD_SECTION(icwmpd, config, section, &s, &add_value);
        if (option)
            DMUCI_SET_VALUE_BY_SECTION(icwmpd, s, option, filter);
        if (option1)
            DMUCI_SET_VALUE_BY_SECTION(icwmpd, s, option1, val1);
        if (option2)
            DMUCI_SET_VALUE_BY_SECTION(icwmpd, s, option2, val2);
        i++;
    }
}
...
bool synchronize_multi_config_sections_with_dmmap_eq(char *package, char *section_type, char *dmmap_package, char *dmmap_section, char *option_name, char *option_value, void *additional_attribute,
...
bool synchronize_multi_config_sections_with_dmmap_eq_diff(char *package, char *section_type, char *dmmap_package, char *dmmap_section, char *option_name, char *option_value, char *opt_diff_name,
                                                          char *opt_diff_value, void *additional_attribute, struct list_head *dup_list)
{
    struct uci_section *s, *stmp, *dmmap_sect;
    FILE               *fp;
...
char *dm_strword(char *src, char *str)
{
    ...
    {
        if ((ret == src && isword_delim(ret[len])) || (ret != src && isword_delim(ret[len]) && isword_delim(*(ret - 1))))
            return ret;
        ret++;
    }
    return NULL;
...
int command_exec_output_to_array(char *cmd, char **output, int *length)
{
    ...
    while (fgets(out, sizeof(out) - 1, fp) != NULL) asprintf(&output[i++], "%s", out);
