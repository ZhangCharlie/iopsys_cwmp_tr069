...
static operation_args ip_diagnostics_iplayercapacity_args = {.in  = (const char *[]){"Interface",
                                                                                     "Role",
                                                                                     "Host",
                                                                                     "Port",
...
int operate_IPDiagnostics_IPLayerCapacity(char *refparam, struct dmctx *ctx, void *data, char *instance, char *value, int action)
{
    ...
    snprintf(input,
             sizeof(input),
             "'{\"host\": "
             "\"%s\",\"interface\":\"%s\",\"role\":\"%s\",\"port\":\"%s\",\"jumbo_frames\":\"%s\",\"proto_ver\":\"%s\",\"udp_content\":\"%s\",\"test_type\":\"%"
             "s\",\"ipdv_enable\":\"%s\",\"DSCP\":\"%s\",\"rate_index\":\"%s\",\"mode_subintervals\":\"%s\",\"test_subinterval\":\"%s\",\"feedback_interval\":"
             "\"%s\",\"seq_err_thresh\":\"%s\",\"dup_ignore\":\"%s\",\"lower_thresh\":\"%s\",\"upper_thresh\":\"%s\",\"high_speed_delta\":\"%s\",\"algorithm\":"
             "\"%s\",\"slow_adj_thresh\":\"%s\",\"num_interval\":\"%s\",\"proto\":\"%s\"}'",
             host,
             interface,
             role,
             port,
             DM_STRLEN(jumbo) > 0 ? (jumbo_en ? "1" : "0") : "\0",
             ip_proto,
             content,
             test_type,
             DM_STRLEN(ipdv) > 0 ? (ipdv_en ? "1" : "0") : "\0",
             dscp,
             start_rate,
             mode_test,
             sub_interval,
             feed_interval,
             seq_err,
             DM_STRLEN(dup_ignore) > 0 ? (dup_ignore_en ? "1" : "0") : "\0",
             low_thresh,
             up_thresh,
             speed_delta,
             rate_adj,
...
DMLEAF tIPLayerCapacityParams[] = {
    ...
    {"NumberFirstModeTestSubIntervals",
     &DMWRITE,
     DMT_UNINT,
     get_IPDiagnosticsIPLayerCapacity_NumberFirstModeTestSubIntervals,
     set_IPDiagnosticsIPLayerCapacity_NumberFirstModeTestSubIntervals,
     BBFDM_CWMP},
