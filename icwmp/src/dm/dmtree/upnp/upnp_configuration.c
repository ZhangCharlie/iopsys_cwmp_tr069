...
DMOBJ upnpConfigurationObj[] = {{"Network", &DMREAD, NULL, NULL, NULL, NULL, &DMFINFRM, &DMNONE, upnpConfigurationNetworkObj, upnpConfigurationNetworkParams, NULL}, {0}};

/*** /UPnP/DM/Configuration/Network objects ***/
DMOBJ upnpConfigurationNetworkObj[] = {{"IPInterface",
                                        &DMWRITE,
                                        upnp_configuration_ipinterface_createinstance,
                                        upnp_configuration_ipinterface_deleteinstance,
                                        NULL,
                                        upnp_browseIPInterfaceInst,
                                        &DMFINFRM,
                                        &DMNONE,
                                        upnpConfigurationNetworkIPInterfaceObj,
                                        upnpConfigurationNetworkIPInterfaceParams,
                                        NULL},
                                       {0}};

/*** /UPnP/DM/Configuration/Network parameters ***/
DMLEAF upnpConfigurationNetworkParams[] = {{"HostName", &DMWRITE, DMT_STRING, upnp_configuration_get_hostname, upnp_configuration_set_hostname, &DMFINFRM, NULL},
...
DMOBJ upnpConfigurationNetworkIPInterfaceObj[] = {{"IPv4", &DMREAD, NULL, NULL, NULL, NULL, &DMFINFRM, &DMNONE, NULL, upnpConfigurationNetworkIPInterfaceIpv4Params, NULL}, {0}};

/*** /UPnP/DM/Configuration/Network/IPInterface/#/ parameters ***/
DMLEAF upnpConfigurationNetworkIPInterfaceParams[] = {{"SystemName", &DMREAD, DMT_STRING, upnp_configuration_get_SystemName, NULL, &DMFINFRM, NULL}, {0}};
...
int upnp_configuration_get_SystemName(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
    ...
    if (upnp_configargs->systemName != NULL && strlen(upnp_configargs->systemName) > 0)
        dmasprintf(value, "%s", upnp_configargs->systemName);
    return 0;
}
...
int upnp_configuration_get_ipv4_addressingtype(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
    ...
    if (proto != NULL && strlen(proto) > 0 && strstr(proto, "dhcp"))
        dmasprintf(value, "%s", "DHCP");
    else
        dmasprintf(value, "%s", "Static");
    return 0;
}
...
int upnp_configuration_get_ipv4_defaultgateway(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
    ...
    if (fp != NULL)
    {
        ...
        while (fgets(line, MAX_PROC_ROUTING, fp) != NULL)
        {
            ...
            {
                if (proute.gateway != NULL && strlen(proute.gateway) > 0)
                    dmasprintf(value, "%s", proute.gateway);
                return 0;
            }
            else
                continue;
...
int upnp_configuration_ipinterface_deleteinstance(char *refparam, struct dmctx *ctx, void *data, char *instance, unsigned char del_action)
{
    ...
    {
        case DEL_INST:
            if (upnp_configargs->upnpConfiguration_sec == NULL)
                return FAULT_9005;
...
int upnp_browseIPInterfaceInst(struct dmctx *dmctx, DMNODE *parent_node, void *prev_data, char *prev_instance)
{
    ...
    uci_foreach_sections("network", "interface", net_sec)
    {
        ...
        if (DM_LINK_INST_OBJ(dmctx, parent_node, ( void * )&currUpnpConfigurationArgs, iface_int) == DM_STOP)
            break;
    }
    return 0;
}
