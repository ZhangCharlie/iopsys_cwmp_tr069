...
DMOBJ upnpMonitoringObj[] = {{"OperatingSystem", &DMREAD, NULL, NULL, NULL, NULL, &DMFINFRM, &DMNONE, NULL, upnpMonitoringOperatingSystemParams, NULL},
...
DMLEAF upnpMonitoringParams[] = {{"IPUsageNumberOfEntries", &DMREAD, DMT_UNINT, upnp_monitoring_get_IPUsageNumberOfEntries, NULL, &DMFINFRM, NULL}, {0}};

/*** /UPnP/DM/Monitoring/OperatingSystem parameters ***/
DMLEAF upnpMonitoringOperatingSystemParams[] = {{"CurrentTime", &DMREAD, DMT_TIME, upnp_monitoring_get_CurrentTime, NULL, &DMFINFRM, NULL},
...
DMLEAF upnpMonitoringExecutionEnvironmentParams[] = {
    {"CPUUsage", &DMREAD, DMT_UNINT, upnp_monitoring_get_CPUUsage, NULL, &DMFINFRM, NULL}, {"MemoryUsage", &DMREAD, DMT_UNINT, upnp_monitoring_get_MemoryUsage, NULL, &DMFINFRM, NULL}, {0}};

/*** /UPnP/DM/Monitoring/Usage parameters ***/
DMLEAF upnpMonitoringIPUsageParams[] = {{"SystemName", &DMREAD, DMT_STRING, upnp_monitoring_get_SystemName, NULL, &DMFINFRM, NULL},
...
int upnp_BrowseIPUsage(struct dmctx *dmctx, DMNODE *parent_node, void *prev_data, char *prev_instance)
{
    ...
    uci_foreach_sections("network", "interface", net_sec)
    {
        ...
        if (status != NULL)
            dmasprintf(&curr_ip_usage_args.status, "%s", status);
        else
            dmasprintf(&curr_ip_usage_args.status, "");
        upnp_getInterfaceTotalPacketSent(section_name(net_sec), &totalPktSnt);
        if (totalPktSnt != NULL)
            dmasprintf(&curr_ip_usage_args.totalpacketsent, "%s", totalPktSnt);
        else
            dmasprintf(&curr_ip_usage_args.totalpacketsent, "");
        upnp_getInterfaceTotalPacketReceived(section_name(net_sec), &totalPktRcvd);
        if (totalPktRcvd != NULL)
            dmasprintf(&curr_ip_usage_args.totalpacketreceived, "%s", totalPktRcvd);
        else
            dmasprintf(&curr_ip_usage_args.totalpacketreceived, "");
        ...
        if (DM_LINK_INST_OBJ(dmctx, parent_node, ( void * )&curr_ip_usage_args, iface_int) == DM_STOP)
            break;
        i++;
    }
    return 0;
