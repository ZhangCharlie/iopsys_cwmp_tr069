...
DMOBJ tWANDeviceObj[] = {
    ...
    // dynamic
    {"WANConnectionDevice",
     &DMWANConnectionDevice,
     add_wan_wanconnectiondevice,
     delete_wan_wanconnectiondevice,
     NULL,
     browsewanconnectiondeviceInst,
     &DMFINFRM,
     &DMWANConnectionDevicenotif,
     tWANConnectionDeviceObj,
     tWANConnectionDeviceParam,
     NULL},
...
DMLEAF tWANDeviceParam[] = {{"Alias", &DMWRITE, DMT_STRING, get_wan_dev_alias, set_wan_dev_alias, NULL, NULL}, {0}};

/*** WANDevice.{i}.WANConnectionDevice.{i}. ***/
DMOBJ tWANConnectionDeviceObj[] = {{"WANIPConnection",
                                    &DMWRITE,
                                    add_wan_wanipconnection,
                                    delete_wan_wanipconnectiondevice,
                                    NULL,
                                    browsewanprotocolconnectionipInst,
                                    &DMFINFRM,
                                    &DMWANConnectionDevicenotif,
                                    tWANConnectionObj,
                                    tWANIPConnectionParam,
                                    get_protocol_connection_linker},
                                   {"WANPPPConnection",
                                    &DMWRITE,
                                    add_wan_wanpppconnection,
                                    delete_wan_wanpppconnectiondevice,
                                    NULL,
                                    browsewanprotocolconnectionpppInst,
                                    &DMFINFRM,
                                    &DMWANConnectionDevicenotif,
                                    tWANConnectionObj,
                                    tWANPPPConnectionParam,
                                    get_protocol_connection_linker},
                                   // Depend on wandev type
                                   {"WANDSLLinkConfig", &DMREAD, NULL, NULL, check_wan_is_atm, NULL, NULL, NULL, NULL, tWANDSLLinkConfigParam, NULL},
                                   {0}};

DMLEAF tWANConnectionDeviceParam[] = {{"Alias", &DMWRITE, DMT_STRING, get_wan_con_dev_alias, set_wan_con_dev_alias, NULL, NULL}, {0}};
...
DMLEAF tWANPPPConnectionParam[] = {{"Alias", &DMWRITE, DMT_STRING, get_wan_ppp_con_alias, set_wan_ppp_con_alias, NULL, NULL},
                                   {"Enable", &DMWRITE, DMT_BOOL, get_interface_enable_wanproto, set_interface_enable_wanproto, NULL, NULL},
...
DMLEAF tWANConnectionStatsParam[] = {{"EthernetBytesReceived", &DMREAD, DMT_UNINT, get_wan_link_connection_eth_bytes_received, NULL, NULL, NULL},
...
DMLEAF tWANConnection_VLANParam[] = {{"VLANID", &DMWRITE, DMT_UNINT, get_wan_ip_link_connection_vid, set_wan_ip_link_connection_vid, NULL, NULL},
...
DMLEAF tWANDSLLinkConfigParam[] = {{"Enable", &DMREAD, DMT_BOOL, get_wan_dsl_link_config_enable, NULL, NULL, NULL},
...
DMLEAF tWANCommonInterfaceConfigParam[] = {{"WANAccessType", &DMREAD, DMT_STRING, get_wan_device_wan_access_type, NULL, NULL, NULL}, {0}};

/*** WANDevice.{i}.WANDSLInterfaceConfig. ***/
DMLEAF tWANDSLInterfaceConfigParam[] = {{"Status", &DMREAD, DMT_STRING, get_wan_device_wan_dsl_interface_config_status, NULL, NULL, NULL},
...
DMLEAF tWANEthernetInterfaceConfigParam[] = {{"Enable", &DMWRITE, DMT_BOOL, get_wan_eth_intf_enable, set_wan_eth_intf_enable, NULL, NULL},
...
DMLEAF tWANEthernetInterfaceConfigStatsParam[] = {{"BytesSent", &DMREAD, DMT_UNINT, get_wan_eth_intf_stats_tx_bytes, NULL, NULL, NULL},
...
struct wan_device wan_devices[WAN_DEVICE] = {[WAN_IDX_ETH] = {"1", "eth0", "ethport", "ports"}, [WAN_IDX_ATM] = {"2", "atm", "atm-device", "dsl"}, [WAN_IDX_PTM] = {"3", "ptm", "ptm-device", "dsl"}};
        ...
        if ((devid != NULL && atoi(devid + 1) > 1) || strstr(pch, "atm") || strstr(pch, "ptm") || strstr(pch, eth_wan))
        {
            uci_foreach_option_eq("network", "device", "ifname", pch, s)
            {
                ...
                if (dmmap_section != NULL)
                    dmuci_set_value_by_section(dmmap_section, "bridge", bridge);
            }
        }
    }
}
...
int delete_wan_wanpppconnectiondevice(char *refparam, struct dmctx *ctx, void *data, char *instance, unsigned char del_action)
{
    ...
    switch (del_action)
    {
        ...
        case DEL_ALL:
            ...
            {
                dmuci_get_value_by_section_string(s, "proto", &iproto);
                if (strstr(iproto, "ppp"))
                { // CHECK IF WE CAN OPTIMISE AND IF iproto can be pppoa
                    if (ss)
                    {
                        get_dmmap_section_of_config_section("dmmap_network", "interface", section_name(ss), &dmmap_section);
                        dmuci_delete_by_section(dmmap_section, NULL, NULL);
...
int get_wan_device_wan_dsl_interface_config_status(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
    ...
    *value = "";
    ...
    else
    {
        ...
        if (!(wandargs->instance == WAN_INST_ATM && dsl == WAN_DSL_ADSL) && !(wandargs->instance == WAN_INST_PTM && dsl == WAN_DSL_VDSL))
        {
            *value = "";
            return 0;
        }
...
int get_layer2_interface(char *wan_name, char **ifname)
{
    ...
    *ifname = "";
    ...
    for (pch = strtok_r(dup, " ", &spch); pch != NULL; pch = strtok_r(NULL, " ", &spch))
    {
        if (strstr(pch, "atm") || strstr(pch, "ptm") || strstr(pch, eth_wan))
        {
            devid = strrchr(pch, '.');
            if (devid != NULL && atoi(devid + 1) >= 1)
            {
    ...
    if (i == WAN_IDX_ETH)
    {
        ...
        list_for_each_entry(p, &dup_list, list)
        {
            if (!strcmp(p->config_section->e.name, "WAN"))
            {
                ...
                {
                    notif_permission = false;
                    if (default_wan_proto == WAN_PROTO_IP)
                        ipn_perm = false;
                    else if (default_wan_proto == WAN_PROTO_PPP)
                        pppn_perm = false;
                }
    ...
    if (containsrepeater)
    {
        ...
        if (repeatindex == 0)
        {
            ...
            uci_foreach_sections("wireless", "wifi-device", s)
            {
                ...
                if (freq[0] == f)
                    dmasprintf(wan_interface, "%s", s->e.name);
            }
        }
        else
        {
