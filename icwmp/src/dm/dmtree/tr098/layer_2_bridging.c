...
struct wan_interface wan_interface_tab[3] = {{"1", "ethernet", "ports", "ethport"}, {"2", "adsl", "dsl", "atm-device"}, {"3", "vdsl", "dsl", "ptm-device"}};
...
DMLEAF tavailableinterfaceParam[] = {{"Alias", &DMWRITE, DMT_STRING, get_avai_int_alias, set_avai_int_alias, NULL, NULL},
...
DMLEAF tlayer2_markingParam[] = {{"Alias", &DMWRITE, DMT_STRING, get_marking_alias, set_marking_alias, NULL, NULL},
...
DMLEAF tlayer2_bridgeParam[] = {{"Alias", &DMWRITE, DMT_STRING, get_bridge_alias, set_bridge_alias, NULL, NULL},
...
DMLEAF tbridge_vlanParam[] = {{"Alias", &DMWRITE, DMT_STRING, get_brvlan_alias, set_brvlan_alias, NULL, NULL},
...
int browselayer2_availableinterfaceInst(struct dmctx *dmctx, DMNODE *parent_node, void *prev_data, char *prev_instance)
{
        ...
        if (i == 0)
        {
            ...
            list_for_each_entry(p, &dup_list, list)
            {
                if (!strcmp(p->config_section->e.name, "WAN"))
                {
                    ...
                    dmasprintf(&oface,
                               "%s%cWANDevice%c%s%cWANConnectionDevice%c%s%c",
                               dmroot,
                               dm_delim,
                               dm_delim,
                               wan_interface_tab[i].instance,
                               dm_delim,
                               dm_delim,
                               waninstance,
                               dm_delim); // MEM WILL BE FREED IN DMMEMCLEAN
    ...
    {
        dmasprintf(&oface, "%s%cLANInterfaces%cLANEthernetInterfaceConfig%c%d%c", dmroot, dm_delim, dm_delim, dm_delim, ++i, dm_delim); // MEM WILL BE FREED IN
                                                                                                                                        // DMMEMCLEAN
...
void init_args_layer2(struct args_layer2 *args, struct uci_section *s, struct uci_section *ss, char *availableinterface_instance, char *bridge_instance, char *interface_type, char *oface)
{
...
void update_add_vlan_to_bridge_interface(char *bridge_key, struct uci_section *dmmap_s)
{
    ...
    uci_path_foreach_option_eq(icwmpd, "dmmap_network", "interface", "bridge_instance", bridge_key, s)
    {
        ...
        uci_path_foreach_option_eq(icwmpd, "dmmap", "vlan_bridge", "bridgekey", bridge_key, marking_bridge_s)
        {
            ...
            if (strncmp(baseifname, wan_baseifname, 4) == 0 || strncmp(baseifname, "ptm", 3) == 0 || strncmp(baseifname, "atm", 3) == 0)
            {
...
int set_marking_bridge_key_sub(char *refparam, struct dmctx *ctx, void *data, char *instance, char *value)
{
    ...
    else if (strncmp(baseifname, wan_baseifname_marking, 4) == 0 || strncmp(baseifname, "ptm", 3) == 0 || strncmp(baseifname, "atm", 3) == 0)
    {
        found = false;
