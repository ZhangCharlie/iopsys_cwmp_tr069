...
DMLEAF tRoutingParam[] = {{"RouterNumberOfEntries", &DMREAD, DMT_UNINT, get_router_nbr_entry, NULL, NULL, NULL}, {0}};
...
DMLEAF tRouterInstParam[] = {{"Enable", &DMWRITE, DMT_BOOL, get_RoutingRouter_Enable, set_RoutingRouter_Enable, NULL, NULL},
...
DMLEAF tIPv4ForwardingParam[] = {{"Enable", &DMRouting, DMT_BOOL, get_router_ipv4forwarding_enable, set_router_ipv4forwarding_enable, NULL, NULL},
...
static bool is_cfg_route_active(struct uci_section *s)
{
    ...
    if (fp != NULL)
    {
        ...
        while (fgets(line, MAX_PROC_ROUTING, fp) != NULL)
        {
            ...
            if (strcmp(dest, proute.destination) == 0 && (mask[0] == '\0' || strcmp(mask, proute.mask) == 0))
            {
                fclose(fp);
                return true;
            }
...
static int dmmap_synchronizeRoutingRouterIPv6Forwarding(struct dmctx *dmctx, DMNODE *parent_node, void *prev_data, char *prev_instance)
{
    ...
    uci_path_foreach_sections_safe(icwmpd, "dmmap_route_forwarding", "route6_dynamic", stmp, s)
    {
        ...
        while (fgets(buf, 512, fp) != NULL)
        {
            if (*buf == '\n' || *buf == '\0')
                continue;
            sscanf(buf, "%8x%8x%8x%8x %x %*s %*s %8x%8x%8x%8x %x %x %x %x %s", &ip[0], &ip[1], &ip[2], &ip[3], &prefix, &gw[0], &gw[1], &gw[2], &gw[3], &metric, &refcnt, &use, &flags, dev);
            if (strcmp(dev, "lo") == 0)
                continue;
            ip[0] = htonl(ip[0]);
            ip[1] = htonl(ip[1]);
