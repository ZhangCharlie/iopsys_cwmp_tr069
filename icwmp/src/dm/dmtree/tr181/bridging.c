        ...
        case VALUESET:
            ...
            if (linker && check_ifname_exist_in_br_ifname_list(linker))
            {
                ...
                if ((( struct bridging_port_args * )data)->ifname[0] == '\0')
                    DMUCI_DELETE_BY_SECTION(icwmpd, (( struct bridging_port_args * )data)->bridge_port_sec, NULL,
                                            NULL); // delete dmmap section after remove br_port_instance to adequate config
            }
            else
            {
                return FAULT_9005;
...
int browseBridgePortInst(struct dmctx *dmctx, DMNODE *parent_node, void *prev_data, char *prev_instance)
{
    ...
    uci_path_foreach_option_eq(icwmpd, "dmmap_bridge_port", "bridge_port", "bridge_key", (( struct bridging_args * )prev_data)->br_key, new_port)
    {
        ...
        if (strcmp(is_dmmap, "false") != 0)
        {
            ...
            port = handle_update_instance(
                2, dmctx, &port_last, update_instance_alias_icwmpd, 5, new_port, "bridge_port_instance", "bridge_port_alias", &find_max, (( struct bridging_args * )prev_data)->br_key);
    ...
    list_for_each_entry(p, &dup_list, list)
    {
        ...
        port = handle_update_instance(
            2, dmctx, &port_last, update_instance_alias_icwmpd, 5, p->dmmap_section, "bridge_port_instance", "bridge_port_alias", &find_max, (( struct bridging_args * )prev_data)->br_key);
        if (DM_LINK_INST_OBJ(dmctx, parent_node, ( void * )&curr_bridging_port_args, port) == DM_STOP)
            goto end;
    }
    free_dmmap_config_dup_list(&dup_list);
