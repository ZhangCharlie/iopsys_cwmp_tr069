...
DMLEAF tWiFiAccessPointAccountingParams[] = {
    ...
    //{"SecondaryServerIPAddr", &DMWRITE, DMT_STRING, get_WiFiAccessPointAccounting_SecondaryServerIPAddr, set_WiFiAccessPointAccounting_SecondaryServerIPAddr,
    // NULL, NULL},
    {"ServerPort", &DMWRITE, DMT_UNINT, get_WiFiAccessPointAccounting_ServerPort, set_WiFiAccessPointAccounting_ServerPort, NULL, NULL},
    //{"SecondaryServerPort", &DMWRITE, DMT_UNINT, get_WiFiAccessPointAccounting_SecondaryServerPort, set_WiFiAccessPointAccounting_SecondaryServerPort, NULL,
    // NULL},
...
static void get_value_security_mode(char **value, char *encryption, char *cipher)
{
    if (strcmp(encryption, "none") == 0)
        ...
        *value = "WPA-Enterprise";
             ...
             || (strcmp(encryption, "psk2") == 0 && *cipher == '\0'))
        *value = "WPA2-Personal";
    else if (strcmp(encryption, "wpa2") == 0)
        *value = "WPA2-Enterprise";
    else if ((strcmp(encryption, "mixed-psk") == 0 && strcmp(cipher, "auto") == 0) || (strcmp(encryption, "mixed-psk") == 0 && strcmp(cipher, "ccmp") == 0)
             || (strcmp(encryption, "mixed-psk") == 0 && strcmp(cipher, "tkip+ccmp") == 0) || (strcmp(encryption, "mixed-psk") == 0 && *cipher == '\0'))
...
{
    adm_entry_get_linker_param(ctx, dm_print_path("%s%cWiFi%cSSID%c", dmroot, dm_delim, dm_delim, dm_delim), (( struct wifi_enp_args * )data)->ifname,
                               value); // MEM WILL BE FREED IN DMMEMCLEAN
    if (*value == NULL)
        *value = "";
    return 0;
}
...
int get_ssid_lower_layer(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
    ...
    {
        adm_entry_get_linker_param(ctx, dm_print_path("%s%cWiFi%cRadio%c", dmroot, dm_delim, dm_delim, dm_delim), (( struct wifi_ssid_args * )data)->linker,
                                   value); // MEM WILL BE FREED IN DMMEMCLEAN
        if (*value == NULL)
            *value = "";
    }
...
{
    adm_entry_get_linker_param(ctx, dm_print_path("%s%cWiFi%cSSID%c", dmroot, dm_delim, dm_delim, dm_delim), (( struct wifi_acp_args * )data)->ifname,
                               value); // MEM WILL BE FREED IN DMMEMCLEAN
    if (*value == NULL)
        *value = "";
    return 0;
...
int addObjWiFiEndPoint(char *refparam, struct dmctx *ctx, void *data, char **instance)
{
    ...
    check_create_dmmap_package("dmmap_wireless");
    instancepara1 = get_last_instance_lev2_icwmpd("wireless", "wifi-iface", "dmmap_wireless", "endpointinstance", "mode", "wet")
                        ? get_last_instance_lev2_icwmpd("wireless", "wifi-iface", "dmmap_wireless", "endpointinstance", "mode", "wet")
                        : "0";
    instancepara2 = get_last_instance_lev2_icwmpd("wireless", "wifi-iface", "dmmap_wireless", "endpointinstance", "mode", "sta")
                        ? get_last_instance_lev2_icwmpd("wireless", "wifi-iface", "dmmap_wireless", "endpointinstance", "mode", "sta")
                        : "0";
