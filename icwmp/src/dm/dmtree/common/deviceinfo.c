...
DMLEAF tMemoryStatusParams[] = {{"Total", &DMREAD, DMT_UNINT, get_memory_status_total, NULL, NULL, NULL}, {"Free", &DMREAD, DMT_UNINT, get_memory_status_free, NULL, NULL, NULL}, {0}};

DMOBJ tProcessEntriesObj[] = {{"Process", &DMREAD, NULL, NULL, NULL, browsePocessEntriesInst, NULL, NULL, NULL, tProcessEntrieParams, NULL}, {0}};

DMLEAF tProcessStatusParams[] = {
    {"CPUUsage", &DMREAD, DMT_UNINT, get_process_cpu_usage, NULL, NULL, NULL}, {"ProcessNumberOfEntries", &DMREAD, DMT_UNINT, get_process_number_of_entries, NULL, NULL, NULL}, {0}};

DMLEAF tProcessEntrieParams[] = {{"PID", &DMREAD, DMT_UNINT, get_process_pid, NULL, NULL, NULL},
...
int get_device_devicelog(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
    *value = "";
    ...
    char *p   = buff;
    while (*p)
    { // TODO to optimize, we can avoid this if the '<' and '>' does not cause problem in the tests.
        if (*p == '<')
        {
            *p = '(';
            if (p == buff || *(p - 1) == '\n')
...
{
    struct process_args *proc_args = ( struct process_args * )data;
    if (proc_args->size != NULL)
        *value = proc_args->size;
    else
        *value = "0";
    return 0;
}
...
{
    struct process_args *proc_args = ( struct process_args * )data;
    if (proc_args->cputime != NULL)
        *value = proc_args->cputime;
    return 0;
}
...
{
    struct process_args *proc_args = ( struct process_args * )data;
    if (strchr(proc_args->state, 'S') != NULL)
        *value = "Sleeping";
    else if (strchr(proc_args->state, 'R') != NULL)
        *value = "Running";
    else if (strchr(proc_args->state, 'T') != NULL)
        *value = "Stopped";
    else if (strchr(proc_args->state, 'D') != NULL)
        *value = "Uninterruptible";
    else if (strchr(proc_args->state, 'Z') != NULL)
        *value = "Zombie";
    else
        *value = proc_args->state;
