...
static void ftp_upload_per_packet(libtrace_packet_t *packet)
{
    ...
    if ((strcmp(tcp_flag, "PSH ACK ") == 0 || strcmp(tcp_flag, "FIN PSH ACK ") == 0)
        && ntohl(tcp->ack_seq) == upload_stats.ack_seq) //&& strlen(nexthdr) > 16 && strncmp(nexthdr, "HTTP/1.1 200 OK", 16) == 0
    {
        ts = trace_get_timeval(packet);
        ( void )localtime_r(&(ts.tv_sec), &lt);
        strftime(s_now, sizeof s_now, "%Y-%m-%dT%H:%M:%S", &lt);
    ...
    if ((strcmp(tcp_flag, "PSH ACK ") == 0 || strcmp(tcp_flag, "FIN PSH ACK ") == 0) && strlen(nexthdr) > strlen(FTP_TRANSFERT_COMPLETE)
        && strncmp(nexthdr, FTP_TRANSFERT_COMPLETE, strlen(FTP_TRANSFERT_COMPLETE)) == 0) //&& strlen(nexthdr) > 16 && strncmp(nexthdr, "HTTP/1.1 200 OK", 16) == 0
    {
        ts = trace_get_timeval(packet);
        ( void )localtime_r(&(ts.tv_sec), &lt);
        strftime(s_now, sizeof s_now, "%Y-%m-%dT%H:%M:%S", &lt);
...
int cwmp_start_diagnostic(int diagnostic_type)
{
    ...
    if (url == NULL || ((url != NULL) && (strcmp(url, "") == 0))
        || ((strncmp(url, DOWNLOAD_PROTOCOL_FTP, strlen(DOWNLOAD_PROTOCOL_FTP)) != 0) && (strstr(url, "@") != NULL && strncmp(url, DOWNLOAD_PROTOCOL_HTTP, strlen(DOWNLOAD_PROTOCOL_HTTP)) == 0)))
    {
