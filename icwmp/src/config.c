...
struct option long_options[] = {{"boot-event", no_argument, NULL, 'b'},
...
int get_global_config(struct config *conf)
{
    ...
    if ((((error == CWMP_OK) && (value != NULL) && (strcmp(value, "enable") == 0)) || ((error2 == CWMP_OK) && ((value2 == NULL) || (value2[0] == 0))))
...
int cwmp_get_deviceid(struct cwmp *cwmp)
{
    ...
    */
    cwmp->deviceid.manufacturer    = strdup("satvix"); // TODO free
    cwmp->deviceid.serialnumber    = strdup("000001");
    cwmp->deviceid.productclass    = strdup("stavix");
    cwmp->deviceid.oui             = strdup("112233");
    cwmp->deviceid.softwareversion = strdup("1.0.0");
