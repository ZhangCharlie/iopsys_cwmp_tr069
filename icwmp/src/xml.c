...
const char *whitespace_cb(mxml_node_t *node, int where)
{
    ...
    switch (where)
    {
        ...
        case MXML_WS_BEFORE_OPEN:
            tab_space[0] = '\0';
            while (node = node->parent) strcat(tab_space, CWMP_MXML_TAB_SPACE);
            return tab_space;
...
int cwmp_rpc_acs_parse_response_inform(struct cwmp *cwmp, struct session *session, struct rpc *this)
{
    ...
    if (b && cwmp->conf.supported_amd_version >= 5)
    { // IF supported version !=5 acs response dosen't contain UseCWMPVersion
        b = mxmlWalkNext(b, tree, MXML_DESCEND_FIRST);
        if (!b || b->type != MXML_TEXT || !b->value.text.string)
            goto error;
...
int cwmp_handle_rpc_cpe_get_parameter_values(struct session *session, struct rpc *rpc)
{
    ...
    while (b)
    {
        if (b && b->type == MXML_TEXT && b->value.text.string && b->parent->type == MXML_ELEMENT && !strcmp(b->parent->value.element.name, "string"))
        {
...
int cwmp_handle_rpc_cpe_get_parameter_names(struct session *session, struct rpc *rpc)
{
    ...
    while (b)
    {
        if (b && b->type == MXML_TEXT && b->value.text.string && b->parent->type == MXML_ELEMENT && !strcmp(b->parent->value.element.name, "ParameterPath"))
        {
            parameter_name = b->value.text.string;
        }
        ...
        if (b && b->type == MXML_TEXT && b->value.text.string && b->parent->type == MXML_ELEMENT && !strcmp(b->parent->value.element.name, "NextLevel"))
        {
...
static int is_duplicated_parameter(mxml_node_t *param_node, struct session *session)
{
    ...
    {
        if (b && b->type == MXML_TEXT && b->value.text.string && b->parent->type == MXML_ELEMENT && !strcmp(b->parent->value.element.name, "Name"))
        {
...
int cwmp_handle_rpc_cpe_set_parameter_values(struct session *session, struct rpc *rpc)
{
    ...
    while (b)
    {
        ...
        if (b && b->type == MXML_TEXT && b->value.text.string && b->parent->type == MXML_ELEMENT && !strcmp(b->parent->value.element.name, "Value"))
        {
            int whitespace;
...
int cwmp_handle_rpc_cpe_set_parameter_attributes(struct session *session, struct rpc *rpc)
{
    ...
    while (b != NULL)
    {
        ...
        if (b && b->type == MXML_TEXT && b->value.text.string && b->parent->type == MXML_ELEMENT && !strcmp(b->parent->value.element.name, "NotificationChange"))
        {
        ...
        if (b && b->type == MXML_TEXT && b->value.text.string && b->parent->type == MXML_ELEMENT && !strcmp(b->parent->value.element.name, "Notification"))
        {
...
int cwmp_handle_rpc_cpe_add_object(struct session *session, struct rpc *rpc)
{
    ...
    while (b)
    {
        if (b && b->type == MXML_TEXT && b->value.text.string && b->parent->type == MXML_ELEMENT && !strcmp(b->parent->value.element.name, "ObjectName"))
        {
            object_name = b->value.text.string;
        }
        if (b && b->type == MXML_TEXT && b->value.text.string && b->parent->type == MXML_ELEMENT && !strcmp(b->parent->value.element.name, "ParameterKey"))
        {
            parameter_key = b->value.text.string;
        }
        b = mxmlWalkNext(b, session->body_in, MXML_DESCEND);
...
int cwmp_handle_rpc_cpe_cancel_transfer(struct session *session, struct rpc *rpc)
{
    ...
    while (b)
    {
        if (b && b->type == MXML_TEXT && b->value.text.string && b->parent->type == MXML_ELEMENT && !strcmp(b->parent->value.element.name, "CommandKey"))
        {
...
int cwmp_handle_rpc_cpe_schedule_inform(struct session *session, struct rpc *rpc)
{
    ...
    while (b)
    {
        ...
        if (b && b->type == MXML_TEXT && b->value.text.string && b->parent->type == MXML_ELEMENT && !strcmp(b->parent->value.element.name, "DelaySeconds"))
        {
...
void *thread_cwmp_rpc_cpe_schedule_download(void *v)
{
    ...
    for (;;)
    {
        ...
        {
            if ((min_time == current_download->timewindowstruct[0].windowstart && (current_download->timewindowstruct[0].windowmode)[0] == '2')
                || (min_time == current_download->timewindowstruct[1].windowstart && (current_download->timewindowstruct[1].windowmode)[0] == '2'))
            {
                pthread_mutex_lock(&mutex_schedule_download);
                external_init( );
                ptransfer_complete       = calloc(1, sizeof(struct transfer_complete));
...
int cwmp_handle_rpc_cpe_change_du_state(struct session *session, struct rpc *rpc)
{
    ...
    while (b != NULL)
    {
        ...
        if (b && b->type == MXML_ELEMENT && 0 == strcmp(b->parent->value.element.name, "Operations"))
        {
            ...
            {
                tmp1 = t;
                if (t && t->type == MXML_ELEMENT && !strcmp(t->parent->value.element.name, "InstallOpStruct"))
                {
                    tmp1 = mxmlWalkNext(tmp1, t, MXML_DESCEND);
                    while (tmp1)
                    {
                        if (tmp1 && tmp1->type == MXML_TEXT && tmp1->value.text.string && tmp1->parent->type == MXML_ELEMENT && !strcmp(tmp1->parent->value.element.name, "URL"))
                        {
                            elem->url = strdup(tmp1->value.text.string);
                        }
                        if (tmp1 && tmp1->type == MXML_TEXT && tmp1->value.text.string && tmp1->parent->type == MXML_ELEMENT && !strcmp(tmp1->parent->value.element.name, "UUID"))
                        {
                            elem->uuid = strdup(tmp1->value.text.string);
                        }
                        if (tmp1 && tmp1->type == MXML_TEXT && tmp1->value.text.string && tmp1->parent->type == MXML_ELEMENT && !strcmp(tmp1->parent->value.element.name, "Username"))
                        {
                            elem->username = strdup(tmp1->value.text.string);
                        }
                        if (tmp1 && tmp1->type == MXML_TEXT && tmp1->value.text.string && tmp1->parent->type == MXML_ELEMENT && !strcmp(tmp1->parent->value.element.name, "Password"))
                        {
                            elem->password = strdup(tmp1->value.text.string);
                        }
                        if (tmp1 && tmp1->type == MXML_TEXT && tmp1->value.text.string && tmp1->parent->type == MXML_ELEMENT && !strcmp(tmp1->parent->value.element.name, "ExecutionEnvRef"))
                        {
                ...
                else if (t && t->type == MXML_ELEMENT && !strcmp(t->parent->value.element.name, "UpdateOpStruct"))
                {
                    tmp1 = mxmlWalkNext(tmp1, t, MXML_DESCEND);
                    while (tmp1)
                    {
                        ...
                        if (tmp1 && tmp1->type == MXML_TEXT && tmp1->value.text.string && tmp1->parent->type == MXML_ELEMENT && !strcmp(tmp1->parent->value.element.name, "Version"))
                        {
                ...
                else if (t && t->type == MXML_ELEMENT && !strcmp(t->parent->value.element.name, "UninstallOpStruct"))
                {
                    tmp1 = mxmlWalkNext(tmp1, t, MXML_DESCEND);
                    while (tmp1)
                    {
...
int cwmp_handle_rpc_cpe_download(struct session *session, struct rpc *rpc)
{
    ...
    while (b != NULL)
    {
        ...
        if (b && b->type == MXML_TEXT && b->value.text.string && b->parent->type == MXML_ELEMENT && !strcmp(b->parent->value.element.name, "FileType"))
        {
            if (download->file_type == NULL)
            {
        ...
        if (b && b->type == MXML_TEXT && b->value.text.string && b->parent->type == MXML_ELEMENT && !strcmp(b->parent->value.element.name, "URL"))
        {
            download->url = strdup(b->value.text.string);
        }
        if (b && b->type == MXML_TEXT && b->value.text.string && b->parent->type == MXML_ELEMENT && !strcmp(b->parent->value.element.name, "Username"))
        {
            download->username = strdup(b->value.text.string);
        }
        if (b && b->type == MXML_TEXT && b->value.text.string && b->parent->type == MXML_ELEMENT && !strcmp(b->parent->value.element.name, "Password"))
        {
            download->password = strdup(b->value.text.string);
        }
        if (b && b->type == MXML_TEXT && b->value.text.string && b->parent->type == MXML_ELEMENT && !strcmp(b->parent->value.element.name, "FileSize"))
        {
    ...
    else if (strncmp(download->url, DOWNLOAD_PROTOCOL_HTTP, strlen(DOWNLOAD_PROTOCOL_HTTP)) != 0 && strncmp(download->url, DOWNLOAD_PROTOCOL_HTTPS, strlen(DOWNLOAD_PROTOCOL_HTTPS)) != 0
...
int cwmp_handle_rpc_cpe_schedule_download(struct session *session, struct rpc *rpc)
{
    ...
    while (b != NULL)
    {
        ...
        if (b && b->type == MXML_ELEMENT && !strcmp(b->parent->value.element.name, "TimeWindowList"))
        {
            ...
            while (t)
            {
                if (t && t->type == MXML_TEXT && t->value.text.string && t->parent->type == MXML_ELEMENT && !strcmp(t->parent->value.element.name, "WindowStart"))
                {
                    schedule_download_delay[j] = atol(t->value.text.string);
                    j++;
                }
                if (t && t->type == MXML_TEXT && t->value.text.string && t->parent->type == MXML_ELEMENT && !strcmp(t->parent->value.element.name, "WindowEnd"))
                {
                    schedule_download_delay[j] = atol(t->value.text.string);
                    j++;
                }
                if (t && t->type == MXML_TEXT && t->value.text.string && t->parent->type == MXML_ELEMENT && !strcmp(t->parent->value.element.name, "WindowMode"))
                {
                    if (schedule_download->timewindowstruct[i].windowmode == NULL)
                    {
                ...
                if (t && t->type == MXML_TEXT && t->value.text.string && t->parent->type == MXML_ELEMENT && !strcmp(t->parent->value.element.name, "UserMessage"))
                {
                    schedule_download->timewindowstruct[i].usermessage = strdup(t->value.text.string);
                }
                if (t && t->type == MXML_TEXT && t->value.text.string && t->parent->type == MXML_ELEMENT && !strcmp(t->parent->value.element.name, "MaxRetries"))
                {
    ...
    if (strcmp(file_type, "1 Firmware Upgrade Image") && strcmp(file_type, "2 Web Content") && strcmp(file_type, "3 Vendor Configuration File") && strcmp(file_type, "4 Tone File")
        && strcmp(file_type, "5 Ringer File") && strcmp(file_type, "6 CWMP CA SSL Certificate File"))
    {
        error = FAULT_CPE_INVALID_ARGUMENTS;
    }
    else if ((strcmp(windowmode0, "1 At Any Time") && strcmp(windowmode0, "2 Immediately") && strcmp(windowmode0, "3 When Idle"))
             || (strcmp(windowmode1, "1 At Any Time") && strcmp(windowmode1, "2 Immediately") && strcmp(windowmode1, "3 When Idle")))
    {
        error = FAULT_CPE_REQUEST_DENIED;
    }
    else if (count_download_queue >= MAX_DOWNLOAD_QUEUE)
...
int cwmp_handle_rpc_cpe_upload(struct session *session, struct rpc *rpc)
{
    ...
    if (strncmp(file_type, "1 Vendor Configuration File", sizeof "1 Vendor Configuration File" - 1) != 0
        && strncmp(file_type, "3 Vendor Configuration File", sizeof "3 Vendor Configuration File" - 1) != 0 && strncmp(file_type, "2 Vendor Log File", sizeof "2 Vendor Log File" - 1) != 0
...
int cwmp_handle_rpc_cpe_fault(struct session *session, struct rpc *rpc)
{
    ...
    u = mxmlNewText(t, 0, (FAULT_CPE_ARRAY[session->fault_code].TYPE == FAULT_CPE_TYPE_CLIENT) ? "Client" : "Server");
    if (!u)
        return -1;
