...
static int jshn_message_parse(char **policy, int size, char **tb, char *msg)
{
    ...
    if (jshn_obj == NULL || is_error(jshn_obj) || json_object_get_type(jshn_obj) != json_type_object)
    {
        jshn_obj = NULL;
        return -1;
    }
    for (i = 0; i < size; i++)
    {
        ...
        if (obj == NULL || is_error(obj) || json_object_get_type(obj) != json_type_string)
            continue;
...
char *dustatechange_fault_policy[] = {[DUState_Change_FAULT] = "fault_code", [DUState_Change_VERSION] = "package_version", [DUState_Change_NAME] = "package_name"};
